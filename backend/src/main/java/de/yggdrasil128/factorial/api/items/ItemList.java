...
public class ItemList {
    ...
    public ItemList(Factory factory, Changelist primary, Iterable<Changelist> active) {
        this.productionSteps = factory.getProductionSteps().stream().map(productionStep -> new ApiProductionStep(
                productionStep, primary, active)).collect(toCollection(ArrayList::new));
        ...
        for (ApiProductionStep productionStep : this.productionSteps) {
            for (Throughput input : productionStep.getInput()) {
                ApiItem apiItem = itemCache.computeIfAbsent(input.getItem(), key -> new ApiItem(key, factory
                        .getItemOrder().get(key)));
            ...
            for (Throughput output : productionStep.getOutput()) {
                ApiItem apiItem = itemCache.computeIfAbsent(output.getItem(), key -> new ApiItem(key, factory
                        .getItemOrder().get(key)));
